...
# Story UI Live Production Start Script - Angular
# Runs Storybook in dev mode + MCP server with Storybook proxy

echo "🚀 Starting Story UI Live Environment (Angular)..."
echo ""
...
# Start Storybook dev server in background (Angular uses ng run)
echo "📖 Starting Angular Storybook dev server on internal port ${STORYBOOK_PORT}..."
...
echo "✅ Angular Storybook dev server running on port ${STORYBOOK_PORT}"
